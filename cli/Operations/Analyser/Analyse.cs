  ...
  public class Analyse {
    ...
    private bool IsMeasureUsed(PbiMeasure m, List<PbiMeasure> all) {
      return layout.IndexOf(m.GetFullName(), StringComparison.OrdinalIgnoreCase) >= 0 ||
             layout.IndexOf($@"\""{m.MEASURE_NAME}\""", StringComparison.OrdinalIgnoreCase) >= 0 || all.Any(m2 =>
                 m2 != m && m2.GetComparableExpression()
                     .IndexOf($"[{m.GetComparableName()}]", StringComparison.OrdinalIgnoreCase) >= 0);
    }
