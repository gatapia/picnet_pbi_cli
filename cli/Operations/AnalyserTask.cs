  ...
  public class AnalyserTask : IRun {
    ...
    public void Run(CliOptions opts) {
      ...
      if (!File.Exists(layoutfile))
        throw new Exception($"the specified directory '{opts.Dir}' does not contain the expected Report\\Layout file");
      var layout = File.ReadAllText(layoutfile);
      var runningpbis = GetSsasPortsAndDb();
      if (!runningpbis.Any())
        throw new Exception(
            "no running Power BI instances found.  Please start the report that matches the code in the current directory.");
      if (runningpbis.Count() > 1)
        throw new Exception(
            "multiple running Power BI instances found.  Please ensure only the single report that matches the code in the current directory is running.");
    ...
    private List<(int port, string dbname)> GetSsasPortsAndDb() {
      return PBI_DIRS.SelectMany(d => {
        return Directory.GetDirectories(d).Select(workspace => Path.Combine(workspace, "Data")).Where(Directory.Exists)
            .Select(datadir => {
              ...
              var port = int.Parse(porttxt);
              var datafile = Directory.GetFiles(datadir, "*.db.xml").SingleOrDefault();
              return datafile == null ? (-1, null) : (port, Path.GetFileName(datafile).Split('.').First());
            });
