  ...
  public class PbixExporter : IRun {
    ...
    public void Run(CliOptions opts) {
      if (string.IsNullOrEmpty(opts.Dir))
        throw new Exception("the export command requires the -o file_name.pbix parameter");

      var dir = new DirectoryInfo(opts.Dir);
      if (!dir.Exists)
        throw new FileNotFoundException($"could not find the specified source directory \"{dir.Name}\"");
      ...
      dir.GetFiles("*.*", SearchOption.AllDirectories).Where(f => f.Name != Constants.GITIGNORE)
          .Where(f => f.Directory?.Name != Constants.DATA_DIR && f.Directory?.Name != Constants.BACKUP_DIR).ToList()
          .ForEach(f => {
            var name = f.FullName.Replace(dir.FullName + "\\", "");

            if (name.EndsWith(".yaml"))
              AddYamlFileToArchive(f, archive, name);
            else if (PbixHelpers.IsValidSourceFile(f))
              AddSourceFileToArchive(f, archive, name);
            else
              AddBinaryFileToArchive(archive, f, dir);
          });
      archive.Save();
    ...
    private void BackupPbixFile(string file, string dir) {
      ...
      new DirectoryInfo(dir).GetFiles("*.pbix").Where(f => (DateTime.Now - f.CreationTime).TotalDays > 5).ToList()
          .ForEach(f => f.Delete());
    }
