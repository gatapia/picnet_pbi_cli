...
namespace cli.Operations {
  public static class PbixHelpers {
    public static bool IsValidSourceFile(FileInfo f) {
      if (Constants.BINARIES.Contains(f.Name) || f.Name == Constants.GITIGNORE ||
          f.Directory?.Name == "data") return false;
    ...
    public static Encoding GetFileEncoding(FileInfo f) {
      return f.Extension == ".xml" ? Encoding.UTF8 :
          f.Extension == ".json" ? Encoding.ASCII : new UnicodeEncoding(false, false);
    }
    ...
    public static string FormatFileContentsImpl(FileInfo f, string contents, bool pretty) {
      ...
      return JsonConvert.SerializeObject(JsonConvert.DeserializeObject(contents),
    ...
    private static object ConvertJTokenToObject(JToken token) {
      object GetFormattedValue(object v) {
        return v is not string str || !str.StartsWith('{')
            ? v
            : JsonConvert.SerializeObject(JsonConvert.DeserializeObject(str), Formatting.Indented)
                .Replace("\r\n", "\n");
      }
      ...
      return token switch {
          ...
          JObject => token.AsEnumerable().Cast<JProperty>()
